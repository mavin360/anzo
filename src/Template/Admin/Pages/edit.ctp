...
<?php //pr($page);?>
 ...
 <?= $this->Form->create($page,['class'=>'form-horizontal form-bordered','enctype' => 'multipart/form-data']);
	...
	<label class="col-sm-2 control-label"> Header Image </label> <?php echo $this->Form->control('page_header_image_new',['class'=>'form-control','type'=>'file']);?>
	<div class="col-sm-3">
	<?php echo $this->Form->control('page_header_image',['type'=>'hidden']);?>
	...
	<?php $i=1;if(!empty($page->sections)){
			foreach($page->sections as $section){
		?>
	
	<div class="new-section">
	<div class="form-group">
	<label class="col-sm-2 control-label">Section Title</label><?php  
	
	$this->Form->templates([
					'inputContainer' => '<div class="col-sm-6">{{content}}</div>',
					'label'=>false
				]);
	echo $this->Form->control('pageSections.section_title[]',['class'=>'form-control','required'=>true,'value'=>$section->section_title]);?>
	<a href="javascript:void(0)" class="panel-close" onClick="close_section(this);">×</a>
	</div>
	<div class="form-group">
	<label class="col-sm-2 control-label">Section Descriptions</label><?php  
	$this->Form->templates([
					'inputContainer' => '<div class="col-sm-9">{{content}}</div>',
					'label'=>false
				]);
	echo $this->Form->control('pageSections.section_text[]',['class'=>'form-control text-editor','type'=>'textarea','value'=>$section->section_text]);?>
	</div>
	</div>
	...
	echo $this->Form->control('selected_module',['options' => $modules,'class'=>'form-control','onChange'=>'module();']);?></div>
	<div id="mID">
	</div>
